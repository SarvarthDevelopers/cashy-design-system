...
export interface FieldItemData {
  ...
  buttonLabel?: string;
  maxFileSize?: number;
  maxFiles?: number;         // New: Max number of files
  acceptedFormats?: string;
  enableCamera?: boolean;
  allowMultiple?: boolean;   // New: For dropdowns/checkboxes
  minLength?: number;        // New: For text
  maxLength?: number;        // New: For text
}
...
const TrashIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M19 5H17H15H9H7H5C4.44772 5 4 5.44772 4 6V7H6H18H20V6C20 5.44772 19.5523 5 19 5Z" fill="currentColor"/>
  </svg>
);
...
const TextExpandedBody = ({ field, onUpdate }: { field: FieldItemData; onUpdate: (id: string, updates: Partial<FieldItemData>) => void }) => (
...
const FileExpandedBody = ({ field, onUpdate }: { field: FieldItemData; onUpdate: (id: string, updates: Partial<FieldItemData>) => void }) => (
  <div className="fi-body-content">
    <div className="fi-section">
      <div className="fi-section__title">General</div>
      <div className="fi-section__fields">
    ...
    <div className="fi-section">
      ...
      <div className="fi-section__fields fi-section__fields--cols2">
        <div className="fi-field-group">
          ...
          <input className="fi-input" type="number" value={field.maxFileSize ?? ''} onChange={e => onUpdate(field.id, { maxFileSize: parseInt(e.target.value) || undefined })} />
        </div>
        <div className="fi-field-group">
          <label className="fi-label">Max Number of Files</label>
          <input className="fi-input" type="number" value={field.maxFiles ?? ''} onChange={e => onUpdate(field.id, { maxFiles: parseInt(e.target.value) || undefined })} />
        </div>
        <div className="fi-field-group">
...
const DropdownExpandedBody = ({ field, onUpdate }: { field: FieldItemData; onUpdate: (id: string, updates: Partial<FieldItemData>) => void }) => {
...
export const FieldItem: React.FC<FieldItemProps> = ({
  ...
  const getSubtitle = () => {
    ...
    switch (type) {
      case 'text':
      case 'textarea':
        if (field.minLength || field.maxLength) info = `${field.minLength || 0}-${field.maxLength || '∞'} chars`;
      ...
      case 'file':
      case 'image':
        info = `${field.maxFiles || 1} file(s) • ${field.maxFileSize || 5}MB`;
        if (type === 'image' && field.enableCamera) info += ' • Camera On';
        break;
      case 'url':
        info = 'URL Validation';
        break;
      case 'date':
        ...
        break;
      default:
        info = field.required ? 'Required' : 'Optional';
    }
    
    return `${typeLabel} • ${info}`;
  };
        ...
        <div className="fi-header__actions">
          {!field.expanded && (
            <div className="fi-required-wrap">
              ...
              <button className={`fi-pill-toggle ${field.required ? 'fi-pill-toggle--on' : ''}`} onClick={handleRequiredToggle}>
        ...
        <div className="fi-body">
          <div className="fi-body__inner">
            ...
            <div className="fi-footer">
              <button className="fi-required-footer" onClick={handleRequiredToggle}>
                <span className="fi-required-text">Required</span>
